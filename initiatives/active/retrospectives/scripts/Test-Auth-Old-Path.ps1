# Test Jira authentication with old credentials path
...
Write-Host "Reading credentials from OLD path:" -ForegroundColor Yellow
Write-Host "  $credPath" -ForegroundColor Gray
Write-Host ""

if (-not (Test-Path $emailPath)) {
    Write-Host "ERROR: Email file not found at $emailPath" -ForegroundColor Red
    exit 1
}
